...
const steps = [
  {
    ...
    icon: Settings,
    codeSnippet: `BATCH_SIZE    = 4096    # M4 Max: 128GB unified memory handles large batches\nEPOCHS        = 50\nLEARNING_RATE = 2e-4    # Lowered: larger model needs smaller LR for stability\nNUM_LAYERS    = 10      # Increased: more expressive distribution learning\nHIDDEN_DIM    = 512     # Increased: richer internal representations\nNUM_BINS      = 12      # Increased: finer-grained spline segments\nBOUND         = 5.0     # Box boundary [-5, 5]`,
    description: "All seven training hyperparameters are declared as uppercase constants at the very top of the file. Every downstream object — DataLoaders, the model, and the trainer — reads from these constants. Values are tuned for the M4 Max with 128GB unified memory.",
  ...
  },
  {
    ...
    why: "Because the split is done at preprocessing time (not here), train.py never touches raw data — it just wraps the ready-made files. Setting num_workers=12 allows 12 CPU threads to pre-fetch batches while the GPU trains. pin_memory is disabled on MPS because Apple Silicon uses unified memory — there is no separate CPU/GPU memory to pin.",
  ...
  },
  {
    ...
    icon: GitMerge,
    codeSnippet: `# Peek at one sample to measure shapes dynamically\nsample        = train_dataset[0]\ndim_theta     = sample['theta'].shape[0]      # → 12\ndim_condition = sample['condition'].shape[0]  # → 3  (x only)\n\nmodel = PipelineConditionalFlow(\n    dim_theta=dim_theta,\n    dim_condition=dim_condition,\n    num_layers=NUM_LAYERS,\n    hidden_dim=HIDDEN_DIM,\n    num_bins=NUM_BINS,\n    bound=BOUND\n)`,
    description: "Instead of hard-coding tensor dimensions, train.py inspects the first dataset sample to measure them at runtime. dim_theta (12 PCA coefficients) and dim_condition (3 measured-now sensor inputs) are passed directly to the model constructor. Note: u (controls) bypasses the flow entirely per the causal partition — only x conditions the coupling layers.",
    why: "This makes the entire training script data-driven. If you change N_COMPONENTS in preprocessing.py, or add new sensor columns, train.py self-adjusts with zero code changes — the model simply grows.",
